...
So arrayToList could iterate over the array backwards (see the previous exercise) and, for each element, add an object to the list.
...
The recursive version of nth will, similarly, look at an ever smaller part of the “tail” of the list and
at the same time count down the index until it reaches zero, at which point it can return the value property of the node it is looking at.
...
// My solution - recursive version of nth
function nth(list, number) {
  if (list === null) {
    return undefined
  } else if (number === 0) {
...
// Book solution - recursive version of nth
function nth(list, n) {
