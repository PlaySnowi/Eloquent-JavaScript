...
class PGroup {
    ...
    add(value) {
        if (this.has(value)) return this; // if the group already has the value, don't add the value again and return the group as is
        return new PGroup(this.members.concat([value]));
    }

    delete(value) {
        if (!this.has(value)) return this; // if there is no value to delete, return the group as is
