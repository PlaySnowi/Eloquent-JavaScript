...
Your measurement function can then, in a loop, generate new states and count the steps each of the robots takes.
When it has generated enough measurements, it can use console.log to output the average for each robot, which is the total number of steps taken divided by the number of measurements. */

// My solution

function runRobotReturns(state, robot, memory) {
    for (let turn = 0; ; turn++) {
...
function compareRobots(robot1, memory1, robot2, memory2) {
    ...
    for (let i = 0; i < 100; i++) {
        ...
        robot1Turns.push(runRobotReturns(state, robot1, memory1));
        robot2Turns.push(runRobotReturns(state, robot2, memory2));
    }

    const reducer = (accumulator, currentValue) => accumulator + currentValue;

    robot1Avg = robot1Turns.reduce(reducer) / robot1Turns.length;
    robot2Avg = robot2Turns.reduce(reducer) / robot2Turns.length;

    console.log(`Robot1 average: ${robot1Avg}, Robot2 average: ${robot2Avg}`)
}
